...
namespace DURLIB
{
    namespace DateMath
    {
        ...
        // char *monthsEnumToText(Months month)
        // {
        //     switch (month)
        //     {
        //     case Months::January:
...
template <>
struct fmt::formatter<DURLIB::DateMath::DateYMD> : fmt::formatter<std::string>
{
    auto format(DURLIB::DateMath::DateYMD my, format_context &ctx) -> decltype(ctx.out())
    {
        if(my.Month + 1 < 10 && my.Day >= 10)
        {
            return fmt::format_to(ctx.out(), "{0}-0{1}-{2}", my.Year, my.Month + 1, my.Day);
        }
        else if(my.Day < 10 && my.Month + 1 >= 10)
        {
            return fmt::format_to(ctx.out(), "{0}-{1}-0{2}", my.Year, my.Month + 1, my.Day);
        }
        else if(my.Day < 10 && my.Month + 1 < 10)
        {
            return fmt::format_to(ctx.out(), "{0}-0{1}-0{2}", my.Year, my.Month + 1, my.Day);
        }
        else
        {
            return fmt::format_to(ctx.out(), "{0}-{1}-{2}", my.Year, my.Month + 1, my.Day);
        }
    }
};